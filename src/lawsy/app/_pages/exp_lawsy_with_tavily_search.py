...
gpt_4o = dspy.LM("openai/gpt-4o", api_key=os.environ["OPENAI_API_KEY"], max_tokens=8192, temperature=0.0, cache=False)
gpt_4o_mini = dspy.LM(
    "openai/gpt-4o-mini", api_key=os.environ["OPENAI_API_KEY"], max_tokens=2048, temperature=0.0, cache=False
)
    ...
    if query and clicked:
        ...
        with st.status("processing", expanded=True) as status:
            ...
            if go_jp:
                ...
                hits = tavily_search_web_retriever.search(query, k=20)
            run = {}
            for result in hits:
                key = result.url
