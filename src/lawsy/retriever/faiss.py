...
class FaissHNSWRetriever:
    ...
    def search(self, vec: npt.NDArray[np.float32], k: int) -> list[Point]:
        vec = vec / np.linalg.norm(vec)
        cossims, indexs = self.index.search(vec.reshape(1, -1), k=k)  # type: ignore
        return [Point(index=i, score=cossim, meta=self.meta_data[i]) for i, cossim in zip(indexs[0], cossims[0])]
    ...
    @staticmethod
    def create(dim: int, m: int = 16) -> "FaissHNSWRetriever":
        assert dim > 0
        assert m > 0
        return FaissHNSWRetriever(dim=dim, m=m)

    @staticmethod
    def load(path: Path | str) -> "FaissHNSWRetriever":
        return FaissHNSWRetriever(path=path)
