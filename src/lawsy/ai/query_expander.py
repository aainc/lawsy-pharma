...
class GenerateDetailedTopics(dspy.Signature):
    ...
    Web検索結果もふまえ、クエリーに関する法令観点の解説文書を作成するために必要な法令を法令検索しようとしています。
    法令検索はセマンティックサーチできるのでキーワード・短文などで検索可能です。

    解説に必要な法令を適切にヒットさせきるための検索トピックを以下の形式でリストアップしてください。
    ...
    - クエリーの意味を十分汲み取ること
    - クエリーに対する法令解説を行うのに必要な法令が揃うように検索トピックをリストアップしてください
