...
class GenerateDetailedTopics(dspy.Signature):
    """あなたは日本の薬機法（医薬品医療機器等法）に精通した薬事専門家です。
    下記のクエリーに関して、事前にWeb検索をして簡単に下調べしてあります。
    Web検索結果もふまえ、クエリーに関する薬機法・薬事規制の解説・解釈文書を作成するために必要な法令を検索しようとしています。
    法令検索はセマンティックサーチできるのでキーワード・短文などで検索可能です。

    解説・解釈に必要な薬機関連法令を適切にヒットさせきるための検索トピックを以下の形式でリストアップしてください。
    ...
    - クエリーの意味を十分汲み取ること
    - 薬機法・薬事規制に関するクエリーに対する法令解説・解釈を行うのに必要な法令が揃うように検索トピックをリストアップしてください
    - 薬事専門用語（GMP、GCP、GVP、GPSP、製造販売業、治験等）を積極的に使用してください
    - 一般的な用語は薬機法での専門用語を優先してください。薬機法令用語が含まれていない場合は、該当しそうな薬機法令用語や関連用語を補完してください
    - 検索トピックは可能な限り互いに重複せず、個別に調査可能な形にしてください。 **self-contained** であるべきです
    - 検索トピックは薬機法令コーパスに準拠した具体的なものにしてください（短文または具体的な薬機法令用語）
    - 検索精度を高めるため、薬機法での使用が想定される具体的な専門用語や関連する条文番号を含めてください
