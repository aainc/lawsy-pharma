...
class CreateOutline(dspy.Signature):
    """あなたは日本の法令に精通したニュースに敏感な専門家です。収集された情報源をもとに、下記のクエリに対するレポートとして適切なアウトラインと簡潔なタイトルを作成してください。また、最後にレポートのタイトルを作成してください。結論パートは絶対に作成しないでください。

    アウトラインは以下のMarkdownフォーマットに従って作成し、次のルールを厳守すること。
    1. Queryに法令観点での回答・解説を目的とした構成にする。
    2. "# Title" をレポートのタイトルに用いる。
    3. "## Title" をセクションのタイトルとして用いる。
       - 各セクション ("## Title") に対して、必ず2～3個以上のサブセクション ("### Title") を生成すること
       - セクション数はQueryに応じて3～20個の間で作成すること。網羅的な回答が必要な場合は多い方が好ましい
    4. "### Title" をサブセクションのタイトルとして用いる。
       - 各サブセクションには、収集された情報源をできるだけ多く含めるようにし、必ず最低3～5個以上の異なる引用番号を使用すること
       - 同じ法令・条文や類似内容のサブセクションは統合し、他の論点のサブセクションを追加すること
       - サブセクションの引用番号が他と重複する場合、より多様な法令を取り入れるために、アウトライン全体を統合・再編成すること
...
class FixOutline(dspy.Signature):
    ...
    3. "## Title" をセクションのタイトルとして用いる。
       - 各セクション ("## Title") に対して、必ず2～3個以上のサブセクション ("### Title") を生成すること
       - セクション数はQueryに応じて3～20個の間で作成すること。網羅的な回答が必要な場合は多い方が好ましい
       - 条文を列挙する等の網羅的な解説が適切な場合、セクションを10個以上に増やし、できるだけ多くの種類の情報源を使用すること
    4. "### Title" をサブセクションのタイトルとして用いる。
       - 各サブセクションには、収集された情報源をできるだけ多く含めるようにし、必ず最低3～5個以上の異なる引用番号を使用すること
       - 同じ法令・条文や類似内容のサブセクションは統合し、他の論点のサブセクションを追加すること
       - サブセクションの引用番号が他と重複する場合、より多様な法令を取り入れるために、アウトライン全体を統合・再編成すること
