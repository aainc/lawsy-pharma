...
class RefineQuery(dspy.Signature):
    """
    あなたは薬機法（医薬品医療機器等法）に精通した優秀な薬事専門家です。
    下記のユーザーのクエリーにたいして、薬機法・薬事規制の観点からの回答を与えてくれそうな簡潔なクエリーを一つ作ってください。
    なお、作成にあたっては下記を守って下さい。

    - 薬機法・薬事規制の観点からの回答を与えてくれそうなクエリーをつくること
    - 薬事専門用語（GMP、GCP、GVP、GPSP、PMDA等）を適切に使用すること
    - あなたの作ったクエリーは可能な限りユーザーが作ったクエリーの意図と過不足なく一致させること
    - クエリーは日本語で作成すること
    - Web検索に最適化すること（薬機関連サイトでの検索を想定）
    - 簡潔であること
    """
